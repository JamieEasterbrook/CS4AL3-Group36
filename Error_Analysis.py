...
def RMS_val (model,x,y,columns, feedback = False):
    ...
    total_months = 147
    x_val, y_val, x_train, y_train, y_pred = clean_data_split(x,y,model, threshold=0.33, test_size=1, scalar=len(columns))
    ...
    if feedback:
        ...
        print(f"{'Zeros baseline':<20} {MSE_ZEROS:>10.2f} {AIRPORT_MSE / MSE * 100:>14.2f}%")
...
plt.xlim(right=6000)
plt.title('Comparizon of Different Regularization Methods on RNN')
plt.legend()
