...
def print_evaluation_metrics(model:nn.Module, val:SequencedDataset):
    ...
    print(
        f'Baseline: F1 score on all one predictions: {2 * (zeroed_prec * zeroed_recall) / (zeroed_prec + zeroed_recall + 1e-4) :.8f}')
...
MODEL_PATH_CUDA = 'model/rnn_model.pt'
MODEL_PATH_CPU = 'model/rnn_model_cpu.pt'
...
if __name__ == '__main__':
    ...
    if EVALUATION_MODE:
        # Evaluates function metrics
        print('evaluating model...')
        model.load_state_dict(torch.load(MODEL_PATH_CUDA, map_location=torch.device(DEVICE)))
    ...
    es = 'due to early stop' if num_no_improve >= PATIENCE else ''
    print(f'training complete {es}')

    torch.save(best_model_state, MODEL_PATH_CPU)
